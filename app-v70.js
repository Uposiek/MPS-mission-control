// ==========================================================
// MPS OPERATING SYSTEM // PWA v7.0
// MODE: PROD
// ==========================================================

const APP_MODE = "PROD";
const DB_NAME = "MPSMissionControl_v6";
const CACHE_TAG = "mps-v70-prod-20260821";
...
function mergeDefaults(incoming={}){
  return {
    ...
    drivingLicence:{...DEFAULT_DATA.drivingLicence,...(incoming.drivingLicence||{})},
    migration63:{...DEFAULT_DATA.migration63,...(incoming.migration63||{})},
    vehicleData:{...DEFAULT_DATA.vehicleData,...(incoming.vehicleData||{})},
    migration70:{...DEFAULT_DATA.migration70,...(incoming.migration70||{})}
  };
}
...
const BUILD_PL_NAMES = {"OEM timing kit (VVT, chain, tensioner, 2 guides)": "Rozrząd OEM (VVT, łańcuch, napinacz, 2 ślizgi)", "Valve cover gasket": "Uszczelka pokrywy zaworów", "Crankshaft seal": "Uszczelniacz wału", "VVT seal": "Uszczelniacz VVT", "Crank pulley bolt": "Śruba koła pasowego", "Valvoline SynPower 5W40 — final oil": "Valvoline SynPower 5W40 — olej docelowy", "Oil filter x3": "Filtr oleju x3", "Gearbox oil": "Olej do skrzyni biegów", "Cleaning materials": "Środki czyszczące", "Coolant": "Płyn chłodniczy", "Cylinder head rebuild": "Regeneracja głowicy", "Block sleeving": "Tulejowanie bloku", "Piston machining for forged rods": "Frez tłoków pod kute korby", "Forged connecting rods": "Kute korby", "Piston rings": "Pierścienie tłokowe", "Cylinder head bolts": "Śruby głowicy", "Clutch regeneration": "Regeneracja sprzęgła", "Crankshaft support bolts": "Śruby podpory wału", "Oil pump": "Pompa oleju", "Oil pickup": "Smok olejowy", "ACL Race main bearings": "Panewki główne ACL Race", "Cylinder head gasket": "Uszczelka pod głowicę", "Rear crankshaft seal — gearbox side": "Uszczelniacz wału od strony skrzyni", "First-start oil 10W40": "Olej na pierwsze odpalenie 10W40", "Break-in oil": "Olej na dotarcie", "A/C recharge": "Nabicie klimatyzacji", "Exhaust manifold gasket": "Uszczelka kolektora wydechowego", "Power steering fluid": "Płyn wspomagania", "Oil filter housing gasket": "Uszczelka podstawy filtra oleju", "Balance delete kit": "Balance delete kit", "ACL Race rod bearings": "Panewki korbowodowe ACL Race", "Injector regeneration": "Regeneracja wtrysków", "D4D injector washers": "Podkładki pod wtryski D4D", "Breather plate gasket": "Uszczelka płytki odmy", "PCV valve gasket": "Uszczelka pod zaworek PCV", "Intake manifold gasket": "Uszczelka kolektora ssącego", "Thermostat": "Termostat", "Oil pump drive": "Napęd pompy oleju", "Water pump": "Pompa wody", "Coolant rail gasket": "Uszczelka magistrali wodnej", "HPFP pump gasket": "Uszczelka pod pompę HPFP", "Long downpipe": "Długi downpipe", "HPFP internals": "Wkład pompy HPFP", "3\" intake": "Dolot 3\"", "DISI tuning": "Strojenie DISI", "Turbocharger": "Turbo", "EBCS": "EBCS", "52 mm oil temperature gauge": "Zegar temperatury oleju 52 mm", "Sensor adapter": "Podstawka pod czujnik", "Break-in fuel": "Paliwo na dotarcie", "Engine removal, teardown, assembly and installation": "Wymontowanie, rozłożenie, złożenie i zamontowanie silnika", "Oil temperature gauge installation": "Montaż zegara temperatury oleju", "Wheel alignment": "Zbieżność"};
function buildDisplayName(x){const lang=localStorage.getItem("mps_v63_ui_language")||"pl";return lang==="pl"?(BUILD_PL_NAMES[x.name]||x.name):x.name}
...
function ensurePersonalDocsSeed(){
  if(!DB.migration63) DB.migration63={personalDocsSeeded:false};
  if(DB.migration63.personalDocsSeeded) return;

  const seeds=[
    {
      id:"seed_oc_euroins_20270818",
      type:"OC",
      name:"Euroins",
      expiry:new Date(2027,7,18,12,0,0).getTime(),
      cost:614.10,
      note:""
    },
    {
      id:"seed_ac_hestia_20270818",
      type:"AC",
      name:"Hestia",
      expiry:new Date(2027,7,18,12,0,0).getTime(),
      cost:781.00,
  ...
  seeds.forEach(seed=>{
    const exists=DB.documents.some(x =>
      x.id===seed.id ||
      (x.type===seed.type &&
       String(x.name||"").toLowerCase()===seed.name.toLowerCase() &&
       Number(x.expiry)===Number(seed.expiry))
    );
    if(!exists) DB.documents.push(seed);
  });

  if(!DB.drivingLicence?.expiry){
    DB.drivingLicence={
      expiry:new Date(2029,6,14,12,0,0).getTime(),
      note:""
    };
  }

  DB.migration63.personalDocsSeeded=true;
}
...
function migrateV7(){
  if(!DB.migration70) DB.migration70={tyreSchemaDone:false};
  if(!DB.migration70.tyreSchemaDone){
    DB.tyres=(DB.tyres||[]).map((t,i)=>({
      ...t,
      frontPair:t.frontPair||"PAIR A",
      rearPair:t.rearPair||"PAIR B",
      frontTread:t.frontTread!=null?Number(t.frontTread):(t.tread!=null?Number(t.tread):null),
      rearTread:t.rearTread!=null?Number(t.rearTread):(t.tread!=null?Number(t.tread):null),
      rotationIntervalKm:Number(t.rotationIntervalKm||DB.settings.tyreRotationKm||8000),
      rotationAccumulatedKm:Number(t.rotationAccumulatedKm||0),
      rotationHistory:Array.isArray(t.rotationHistory)?t.rotationHistory:[],
      lastRotationDate:t.lastRotationDate||null,
      lastRotationOdo:t.lastRotationOdo??null
    }));
    DB.migration70.tyreSchemaDone=true;
  }
}
...
function upcomingItems(){
 ...
 const tyre=activeTyre();if(tyre){const tr=tyreRotationState(tyre);arr.push({name:"TYRE ROTATION",due:tr.forecast,kmLeft:tr.left,factor:"DISTANCE",source:"tyres",item:tyre})}
...
function tyreActiveSegment(t){
 const cur=effectiveCurrentOdo();
 return t.active&&cur!=null&&t.activeFromOdo!=null?Math.max(0,cur-Number(t.activeFromOdo)):0
}
function tyreMileage(t){return Number(t.accumulatedKm||0)+tyreActiveSegment(t)}
function tyreRotationMileage(t){return Number(t.rotationAccumulatedKm||0)+tyreActiveSegment(t)}
function activeTyre(){return DB.tyres.find(x=>x.active)||null}
function tyreRotationState(t){
 if(!t)return null;
 const interval=Number(t.rotationIntervalKm||DB.settings.tyreRotationKm||8000),used=tyreRotationMileage(t),left=interval-used,rate=drivingRateKmDay();
 const forecast=rate&&rate>0?Date.now()+Math.max(0,left)/rate*msDay:null;
 return {interval,used,left,rate,forecast,progress:interval>0?used/interval:0};
}
async function switchTyre(id){
 const cur=effectiveCurrentOdo();if(cur==null){showToast("Set current odometer first.");return}
 DB.tyres.forEach(t=>{
   if(t.active){
     const seg=tyreActiveSegment(t);
     t.accumulatedKm=Number(t.accumulatedKm||0)+seg;
     t.rotationAccumulatedKm=Number(t.rotationAccumulatedKm||0)+seg;
     t.active=false;t.activeFromOdo=null;
   }
 });
 const selected=DB.tyres.find(x=>x.id===id);
 if(selected){selected.active=true;selected.activeFromOdo=cur;selected.lastSwitchDate=Date.now()}
 await saveData();refreshAll();showToast("Active tyre set changed.")
}
async function saveTyreRotation(v,ctx){
 const t=DB.tyres.find(x=>x.id===ctx?.id);if(!t){showToast("No active tyre set.");return false}
 if(!v.date||!v.odo){showToast("Date and odo required.");return false}
 const odo=Number(v.odo);if(!validateOdo(odo,null,null))return false;
 if(!t.active){showToast("Activate this tyre set first.");return false}
 const seg=t.activeFromOdo!=null?Math.max(0,odo-Number(t.activeFromOdo)):0;
 t.accumulatedKm=Number(t.accumulatedKm||0)+seg;
 const usedBefore=Number(t.rotationAccumulatedKm||0)+seg;
 const frontBefore=t.frontPair||"PAIR A",rearBefore=t.rearPair||"PAIR B";
 const frontTread=v.frontTread!=null?Number(v.frontTread):t.frontTread;
 const rearTread=v.rearTread!=null?Number(v.rearTread):t.rearTread;
 t.rotationHistory=Array.isArray(t.rotationHistory)?t.rotationHistory:[];
 t.rotationHistory.push({id:uid(),date:v.date,odo,kmSincePrevious:usedBefore,frontBefore,rearBefore,frontTread,rearTread,note:v.note||""});
 t.frontPair=rearBefore;t.rearPair=frontBefore;
 t.frontTread=rearTread;t.rearTread=frontTread;
 t.rotationAccumulatedKm=0;t.activeFromOdo=odo;t.lastRotationDate=v.date;t.lastRotationOdo=odo;
 updateCurrentOdo(odo);showToast("Tyres rotated front ↔ rear.");return true
}
...
const TYRE_FIELDS=[
 ...
 {key:"dot",label:"DOT",type:"text"},{key:"frontPair",label:"FRONT PAIR NAME",type:"text"},{key:"rearPair",label:"REAR PAIR NAME",type:"text"},
 {key:"frontTread",label:"FRONT TREAD [MM]",type:"number",step:"0.1"},{key:"rearTread",label:"REAR TREAD [MM]",type:"number",step:"0.1"},
 {key:"rotationIntervalKm",label:"ROTATION INTERVAL [KM]",type:"number"},{key:"note",label:"NOTE",type:"textarea"}
];
const TYRE_ROTATION_FIELDS=[
 {key:"date",label:"DATE",type:"datetime-local"},{key:"odo",label:"ODOMETER [KM]",type:"number"},
 {key:"frontTread",label:"FRONT TREAD BEFORE ROTATION [MM]",type:"number",step:"0.1"},{key:"rearTread",label:"REAR TREAD BEFORE ROTATION [MM]",type:"number",step:"0.1"},
 {key:"note",label:"NOTE",type:"textarea"}
];
const VEHICLE_DATA_FIELDS=[
 {key:"model",label:"MAKE / MODEL",type:"text"},{key:"vin",label:"VIN",type:"text"},{key:"registration",label:"REGISTRATION",type:"text"},{key:"year",label:"YEAR",type:"text"},
 {key:"firstRegistration",label:"FIRST REGISTRATION",type:"datetime-local"},{key:"purchaseDate",label:"PURCHASE DATE",type:"datetime-local"},{key:"purchaseOdo",label:"PURCHASE ODO [KM]",type:"number"},{key:"note",label:"NOTE",type:"textarea"}
];
const PART_FIELDS=[
...
const LICENCE_FIELDS=[
 {key:"expiry",label:"VALID UNTIL",type:"datetime-local"},
 {key:"note",label:"NOTE",type:"textarea"}
];
...
document.querySelectorAll("[data-open-modal]").forEach(btn=>btn.addEventListener("click",()=>{
 ...
 if(id==="tyreModal")openForm("TYRE SET","Mileage and front/rear rotation are tracked while a set is active.",TYRE_FIELDS,{season:"summer",frontPair:"PAIR A",rearPair:"PAIR B",rotationIntervalKm:DB.settings.tyreRotationKm||8000},saveTyre);
 if(id==="vehicleDataModal")openForm("VEHICLE DATA","VIN, registration and basic vehicle identity.",VEHICLE_DATA_FIELDS,{...DB.vehicleData,firstRegistration:DB.vehicleData?.firstRegistration?localDateTimeValue(DB.vehicleData.firstRegistration):"",purchaseDate:DB.vehicleData?.purchaseDate?localDateTimeValue(DB.vehicleData.purchaseDate):""},saveVehicleData);
 ...
 if(id==="licenceModal")openForm(
   "DRIVING LICENCE",
   "Track the expiry date of your driving licence.",
   LICENCE_FIELDS,
   {expiry:DB.drivingLicence?.expiry?localDateTimeValue(DB.drivingLicence.expiry):"",note:DB.drivingLicence?.note||""},
   saveDrivingLicence
 );
}));
...
async function saveTyre(v,ctx){
 ...
 const obj={
   id:ctx?.id||uid(),name:v.name,season:v.season,dot:v.dot||"",note:v.note||"",
   frontPair:v.frontPair||old?.frontPair||"PAIR A",rearPair:v.rearPair||old?.rearPair||"PAIR B",
   frontTread:v.frontTread!=null?Number(v.frontTread):null,rearTread:v.rearTread!=null?Number(v.rearTread):null,
   rotationIntervalKm:Number(v.rotationIntervalKm||DB.settings.tyreRotationKm||8000),
   active:old?.active||false,activeFromOdo:old?.activeFromOdo??null,accumulatedKm:Number(old?.accumulatedKm||0),
   rotationAccumulatedKm:Number(old?.rotationAccumulatedKm||0),rotationHistory:Array.isArray(old?.rotationHistory)?old.rotationHistory:[],
   lastSwitchDate:old?.lastSwitchDate||null,lastRotationDate:old?.lastRotationDate||null,lastRotationOdo:old?.lastRotationOdo??null
 };
 upsert("tyres",obj);showToast("Tyre set saved.");return true
}
async function saveVehicleData(v){
 DB.vehicleData={
   model:v.model||"",vin:(v.vin||"").toUpperCase(),registration:(v.registration||"").toUpperCase(),year:v.year||"",
   firstRegistration:v.firstRegistration||null,purchaseDate:v.purchaseDate||null,purchaseOdo:v.purchaseOdo!=null?Number(v.purchaseOdo):null,note:v.note||""
 };
 showToast("Vehicle data saved.");return true
}
...
async function saveDrivingLicence(v){
 if(!v.expiry){showToast("Expiry date required.");return false}
 DB.drivingLicence={expiry:v.expiry,note:v.note||""};
 showToast("Driving licence saved.");
 return true
}
...
function renderService(){
 ...
 renderTyres();renderUpcoming();renderMaintenance();renderServiceHistory()
}
...
$("rotateTyresBtn").addEventListener("click",()=>{
 const t=activeTyre();if(!t){showToast("No active tyre set.");return}
 const odo=effectiveCurrentOdo()||"";
 openForm("ROTATE FRONT ↔ REAR","Records mileage and swaps the two axle pairs.",TYRE_ROTATION_FIELDS,{date:localDateTimeValue(),odo,frontTread:t.frontTread??"",rearTread:t.rearTread??""},saveTyreRotation,t)
});
...
function renderGarage(){
 renderIssues();renderBuildSheet();renderParts()
}
...
function renderTyres(){
 const active=activeTyre(),el=$("tyreList"),a=[...DB.tyres];
 $("activeTyreBadge").textContent=active?active.name:"NO ACTIVE SET";
 const rotateBtn=$("rotateTyresBtn");rotateBtn.disabled=!active;rotateBtn.style.opacity=active?"1":".45";

 if(active){
   const st=tyreRotationState(active),left=Math.max(0,st.left),progress=clamp(st.progress,0,1);
   $("tyreTotalKm").textContent=fmtNum(tyreMileage(active));
   $("tyreRotationKm").textContent=fmtNum(st.used);
   $("tyreRotationLeft").textContent=st.left<0?`${fmtNum(Math.abs(st.left))} OVER`:fmtNum(left);
   $("tyreRotationLeft").className=st.left<=0?"tyreOverdue":st.left<1000?"tyreDue":"";
   $("tyreRotationForecast").textContent=st.left<=0?"DUE NOW":st.forecast?formatDate(st.forecast):"—";
   $("tyreRotationForecastSub").textContent=st.rate?`${fmtNum(st.rate,1)} KM/DAY // ${fmtNum(st.interval)} KM INTERVAL`:"ADD MORE ODO HISTORY";
   $("tyreRotationBar").style.width=`${progress*100}%`;
   $("tyreRotationBar").style.background=st.left<=0?"#ff453a":st.left<1000?"#ff9f35":"linear-gradient(90deg,#25b943,#4be56a)";
   $("tyreFrontPair").textContent=active.frontPair||"PAIR A";$("tyreRearPair").textContent=active.rearPair||"PAIR B";
   $("tyreFrontTread").textContent=active.frontTread!=null?`${fmtNum(active.frontTread,1)} MM`:"—";$("tyreRearTread").textContent=active.rearTread!=null?`${fmtNum(active.rearTread,1)} MM`:"—";
 } else {
   ["tyreTotalKm","tyreRotationKm","tyreRotationLeft","tyreRotationForecast","tyreFrontTread","tyreRearTread"].forEach(id=>$(id).textContent="—");
   $("tyreFrontPair").textContent="PAIR A";$("tyreRearPair").textContent="PAIR B";$("tyreRotationForecastSub").textContent="—";$("tyreRotationBar").style.width="0%";
 }

 if(!a.length){el.className="dataList emptyState";el.textContent="BRAK DANYCH"}
 else{
   el.className="dataList";el.innerHTML=a.map(x=>`<div class="dataRow"><div><div class="dataMain">${x.season==="winter"?"❄️":x.season==="summer"?"☀️":"🛞"} ${escapeHtml(x.name)} ${x.active?'<span class="statusPill green">ACTIVE</span>':""}</div><div class="dataSub">${x.dot?`DOT ${escapeHtml(x.dot)} // `:""}${fmtNum(tyreMileage(x))} KM LOGGED // ROTATION ${fmtNum(tyreRotationMileage(x))}/${fmtNum(x.rotationIntervalKm||DB.settings.tyreRotationKm||8000)} KM${x.note?` // ${escapeHtml(x.note)}`:""}</div></div><div><div class="rowActions">${!x.active?`<button class="miniBtn" onclick="switchTyre('${x.id}')">SWITCH</button>`:""}<button class="miniBtn" onclick="editItem('tyres','${x.id}')">EDIT</button><button class="miniBtn danger" onclick="deleteItem('tyres','${x.id}')">DEL</button></div></div></div>`).join("")
 }

 const hist=active&&Array.isArray(active.rotationHistory)?[...active.rotationHistory].sort((a,b)=>b.date-a.date):[];
 $("tyreRotationHistoryBadge").textContent=`${hist.length} EVENTS`;
 const he=$("tyreRotationHistory");
 if(!hist.length){he.className="dataList emptyState";he.textContent="BRAK DANYCH"}
 else{he.className="dataList";he.innerHTML=hist.slice(0,10).map(h=>`<div class="dataRow"><div><div class="dataMain">↕ ${escapeHtml(h.frontBefore)} ↔ ${escapeHtml(h.rearBefore)}</div><div class="dataSub">${formatDateTime(h.date)} // ${fmtNum(h.odo)} KM // ${fmtNum(h.kmSincePrevious)} KM SINCE PREVIOUS${h.note?` // ${escapeHtml(h.note)}`:""}</div></div></div>`).join("")}
}
function renderBuildSheet(){
 ...
 const rows=DB.buildSheet.filter(x=>{
   const okQ=!q||JSON.stringify(x).toLowerCase().includes(q)||(BUILD_PL_NAMES[x.name]||"").toLowerCase().includes(q);
 ...
 el.innerHTML=rows.map(x=>{
   ...
   return `<div class="dataRow">
     <div>
       <div class="dataMain">▣ ${escapeHtml(buildDisplayName(x))}
...
function renderDrivingLicence(){
 const x=DB.drivingLicence||{},expiry=Number(x.expiry)||null;
 const badge=$("licenceBadge"),dateEl=$("licenceExpiry"),timeEl=$("licenceTimeLeft"),noteEl=$("licenceNote");

 if(!expiry){
   badge.textContent="NO DATE";
   badge.className="tinyBadge";
   dateEl.textContent="—";
   timeEl.textContent="—";
   noteEl.textContent=x.note||"";
   return;
 }

 const days=Math.ceil((expiry-Date.now())/msDay);
 dateEl.textContent=formatDate(expiry);

 if(days<0){
   badge.textContent="EXPIRED";
   badge.className="tinyBadge";
   timeEl.textContent=`${Math.abs(days)}d OVER`;
   timeEl.classList.add("red");
 }
 else{
   badge.textContent="VALID";
   badge.className="tinyBadge greenBadge";
   timeEl.textContent=`${days}d`;
...
// ==========================================================
// AUTO / VEHICLE / CAR HEALTH
// ==========================================================

function nearestDocument(){
 const docs=[...DB.documents].filter(x=>Number(x.expiry));
 if(DB.drivingLicence?.expiry)docs.push({type:"DRIVING LICENCE",name:"",expiry:Number(DB.drivingLicence.expiry),cost:0,note:""});
 return docs.sort((a,b)=>a.expiry-b.expiry)[0]||null
}
function renderVehicleData(){
 const v=DB.vehicleData||{};
 $("vehicleModel").textContent=v.model||"—";$("vehicleYear").textContent=v.year||"—";$("vehicleVin").textContent=v.vin||"—";$("vehicleRegistration").textContent=v.registration||"—";
 $("vehicleFirstReg").textContent=v.firstRegistration?formatDate(v.firstRegistration):"—";$("vehiclePurchaseDate").textContent=v.purchaseDate?formatDate(v.purchaseDate):"—";$("vehiclePurchaseOdo").textContent=v.purchaseOdo!=null?`${fmtNum(v.purchaseOdo)} KM`:"—";$("vehicleDataNote").textContent=v.note||"";
}
function renderCarHealth(){
 const oil=oilForecast(),tyre=activeTyre(),tr=tyre?tyreRotationState(tyre):null,doc=nearestDocument(),issues=DB.issues.filter(x=>x.status!=="resolved"),up=upcomingItems()[0],odo=effectiveCurrentOdo();
 $("healthOdo").textContent=odo!=null?fmtNum(odo):"—";
 if(oil){$("healthOil").textContent=`${fmtNum(Math.max(0,oil.left))} KM`;$("healthOilSub").textContent=`EST. ${formatDate(oil.forecast)}`}else{$("healthOil").textContent="—";$("healthOilSub").textContent="NO BASELINE"}
 if(tr){$("healthTyres").textContent=tr.left<=0?"DUE NOW":`${fmtNum(Math.max(0,tr.left))} KM`;$("healthTyresSub").textContent=tr.forecast?`EST. ${formatDate(tr.forecast)}`:"NO DATE FORECAST"}else{$("healthTyres").textContent="—";$("healthTyresSub").textContent="NO ACTIVE SET"}
 if(doc){const days=Math.ceil((doc.expiry-Date.now())/msDay);$("healthDocument").textContent=days<0?`${Math.abs(days)}d OVER`:`${days}d`;$("healthDocumentSub").textContent=`${doc.type} // ${formatDate(doc.expiry)}`}else{$("healthDocument").textContent="—";$("healthDocumentSub").textContent="NO DOCUMENTS"}
 $("healthIssues").textContent=String(issues.length);$("healthIssuesSub").textContent=issues.length===0?"NONE":issues.some(x=>x.severity==="urgent")?"URGENT ISSUE":"OPEN / OBSERVING";
 if(up){$("healthService").textContent=up.kmLeft!=null?`${fmtNum(Math.max(0,up.kmLeft))} KM`:up.due?formatDate(up.due):"—";$("healthServiceSub").textContent=up.name}else{$("healthService").textContent="—";$("healthServiceSub").textContent="NO UPCOMING"}
 const alert=(oil&&oil.left<=0)||(tr&&tr.left<=0)||issues.some(x=>x.severity==="urgent")||(doc&&doc.expiry<Date.now());
 const warn=!alert&&((oil&&oil.left<1000)||(tr&&tr.left<1000)||issues.length>0||(doc&&doc.expiry-Date.now()<30*msDay));
 const b=$("healthBadge");b.className="tinyBadge"+(alert?"":warn?"":" greenBadge");b.textContent=alert?"ATTENTION":warn?"CHECK":"OK";
}
function renderMissionArchiveCard(){
 const b=$("missionArchiveBadge"),s=$("missionArchiveSummary");
 if(DB.pickup.completed){b.textContent="ARCHIVED";b.className="tinyBadge greenBadge";s.textContent=`Pickup: ${formatDateTime(DB.pickup.date)} // ${fmtNum(DB.pickup.odo)} KM. Full countdown remains available as archive.`}
 else{b.textContent="ACTIVE";b.className="tinyBadge";s.textContent="Rebuild countdown is still active. Vehicle data and documents can already be maintained here."}
}
function renderAuto(){renderCarHealth();renderVehicleData();renderDocuments();renderDrivingLicence();renderMissionArchiveCard()}
...
$("saveTyreSettingsBtn").addEventListener("click",async()=>{
 const km=Number($("settingTyreRotation").value);if(km<=0){showToast("Interval must be > 0.");return}
 DB.settings.tyreRotationKm=km;await saveData();refreshAll();showToast("Tyre rotation settings saved.")
});
...
$("confirmPickupBtn").addEventListener("click",async()=>{
 ...
 await saveData();$("missionCompleteModal").hidden=true;refreshAll();showScreen("auto");showToast("POST-REBUILD OPERATIONS ONLINE 😎")
});
...
$("exportBackupBtn").addEventListener("click",()=>{
 const blob=new Blob([JSON.stringify({app:"MPS Operating System",version:7,mode:APP_MODE,exportedAt:new Date().toISOString(),data:DB},null,2)],{type:"application/json"}),u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download=`MPS_v7_${APP_MODE}_backup_${new Date().toISOString().slice(0,10)}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),3000)
});
...
function demoData(){
 ...
 return mergeDefaults({
  ...
  tyres:[
   {id:uid(),name:"SUMMER SET",season:"summer",dot:"1225",frontPair:"PAIR A",rearPair:"PAIR B",frontTread:6.2,rearTread:6.5,rotationIntervalKm:8000,rotationAccumulatedKm:0,rotationHistory:[],note:"Demo",active:true,activeFromOdo:168420,accumulatedKm:0,lastSwitchDate:pickup,lastRotationDate:pickup,lastRotationOdo:168420},
   {id:uid(),name:"WINTER SET",season:"winter",dot:"4024",frontPair:"PAIR A",rearPair:"PAIR B",frontTread:7.2,rearTread:7.2,rotationIntervalKm:8000,rotationAccumulatedKm:0,rotationHistory:[],note:"Demo",active:false,activeFromOdo:null,accumulatedKm:0,lastSwitchDate:null,lastRotationDate:null,lastRotationOdo:null}
  ],
  parts:[
...
function configureHomeNav(){
 const btn=$("homeNavBtn"),label=$("homeNavLabel");
 if(DB.pickup.completed){btn.dataset.screen="auto";label.textContent="AUTO";$("completeMissionBtn").hidden=true;$("openAutoFromMissionBtn").hidden=true;$("backToAutoBtn").hidden=false}
 else{btn.dataset.screen="mission";label.textContent="MISSION";$("completeMissionBtn").hidden=false;$("openAutoFromMissionBtn").hidden=false;$("backToAutoBtn").hidden=true}
}
$("openAutoFromMissionBtn").addEventListener("click",()=>showScreen("auto"));
$("backToAutoBtn").addEventListener("click",()=>showScreen("auto"));
$("openMissionArchiveBtn").addEventListener("click",()=>showScreen("mission"));
...
function showScreen(name){
 document.querySelectorAll(".screen").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".navBtn").forEach(x=>x.classList.remove("active"));$(`screen-${name}`).classList.add("active");
 const nav=document.querySelector(`.navBtn[data-screen="${name}"]`);if(nav)nav.classList.add("active");else if(name==="mission"&&DB?.pickup?.completed)$("homeNavBtn").classList.add("active");
 if(name==="auto")renderAuto();if(name==="drive")renderDrive();if(name==="service")renderService();if(name==="garage")renderGarage();if(name==="log")renderVehicleLog();if(name==="settings")syncSettings();window.scrollTo({top:0,behavior:"instant"})
}
document.querySelectorAll(".navBtn").forEach(b=>b.addEventListener("click",()=>showScreen(b.dataset.screen)));

function renderExpenses(){}
function refreshAll(){configureHomeNav();syncSettings();renderAuto();renderDrive();renderService();renderGarage();renderVehicleLog()}

async function boot(){
 try{
  await initStorage();syncSettings();refreshAll();updateMission();showScreen(DB.pickup.completed?"auto":"mission");setInterval(updateMission,100);
...
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("sw-v70.js").catch(()=>{}));
